        ...
        <permission-info :value="auth.permissions.value.update_project_settings">
          ...
          >
            <template #dialog-text>
              <p class="mt-0">
                {{ $t('Mark {count} projects as active and allow editing?', { count: selectedItems.length }) }}
              </p>
              <ul class="mt-0">
...
async function setReadonlySelected(projects: PentestProject[]) {
  await bulkAction(projects, p => projectStore.setReadonly(p, false), p => t('Failed to activate "{name}"', { name: p.name }));
  await listViewRef.value?.refresh();
}
</script>
